...
| Docker for integration tests | Docker 29.7.2 present and the daemon responds. `mcr.microsoft.com/mssql/server:2022-latest` has been pulled locally, so Testcontainers starts without a first-run download. | Testcontainers is the default. A `USERMANAGEMENT_TEST_SQL` environment variable overrides it with a connection string - the fallback for a machine without Docker, and what the containerised test run uses so the Docker socket stays out of the test container. It gives each fixture its own database; sharing one broke 122 of 153 tests. |
