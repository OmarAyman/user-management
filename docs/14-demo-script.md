...
| **4:04** | Close | "437 automated tests: backend unit and integration against real SQL Server, Angular component tests, and a Playwright browser suite covering accessibility and responsive layout - plus 66 Postman assertions. Verified from a clean clone, and the whole stack runs on one Docker command." |
...
**The closing number keeps moving as fixes bring their own tests: it is now 437** - 114 unit, 158 integration,
81 Angular, 84 browser. Say the current figure or say "over four hundred"; a number an assessor can check and find wrong is
...
```bash
dotnet test                     # 114 unit + 158 integration, against real SQL Server
npm test --prefix frontend      # 81
npm run e2e --prefix frontend   # 84, in a real browser
```
