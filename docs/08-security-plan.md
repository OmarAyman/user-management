...
| T-03 | Token theft via XSS | anonymous | Access token in memory only, never in `localStorage`/`sessionStorage`; refresh token `httpOnly`; Angular escaping; and a real CSP on the **document** - `script-src 'self'` with no exemption, `frame-ancestors 'none'`, and no third-party origin anywhere in the policy, which is only possible because the fonts are bundled rather than fetched from Google. Until phase 10 the only CSP was the API's `default-src 'none'`, which is correct for JSON and irrelevant to the one response that can execute script; the page carried no security headers at all. Two lint rules make the next two points enforceable rather than customary: `bypassSecurityTrust*` is refused anywhere in the codebase, and `localStorage.setItem` is refused outside `LocaleService`, the only code allowed to persist anything. `npm run lint:verify` proves both fire | An XSS foothold can still call the API as the user while the page is open. Accepted; the fix is to not have XSS |
...
| T-15 | PII over-collection in logs | insider | No code path passes a credential or token to a logger, and request bodies are not logged; `SensitiveDataLoggingTests` runs four sign-in outcomes and a password change through a capturing logger and asserts that the password, the stored hash, the access token and the refresh token appear in neither the rendered message nor any structured value. (An earlier version of this row claimed a Serilog destructuring policy that did not exist - the code simply never logged a credential, which was true and one helpful log line away from false.) Audit rows store names and emails because that is the feature's purpose | Audit rows contain personal data by design; retention policy is a deployment decision, flagged as a known limitation |
